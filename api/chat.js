export default async function handler(req, res) {
  ...
  if (!target) {
    return res.status(400).json({ error: 'No server URL set. Use /url in the app.' })
  }
  if (!user || !pass) {
    return res.status(401).json({ error: 'No credentials set. Use /login in the app.' })
  }
  ...
  // Step 2 — POST to webhook with session cookie
  try {
    ...
    if (ct.includes('text/html')) {
      return res.status(401).json({ error: 'Still seeing login page — credentials may be wrong.' })
    }
